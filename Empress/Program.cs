...
{
    string message;
    message = $"Nome: {empress[i].Name} ";
    message += $"Data Nasc: {empress[i].Born.ToShortDateString()} ";
    Console.WriteLine(message);
}

// LAÇO DE REPETIÇÃO PARA CADA
Console.WriteLine("\n---FOREACH---\n");
foreach( Empresss empresss in empress )
{
    string message = string.Empty;
    if( empress != null)
    {
        message += $"Nome: {empresss.Name}";
    }
    Console.WriteLine(message);
}
